...
\vspace{10pt}
\begin{tcolorbox}[colback=white, colframe=ianColor, arc=2pt, title=\textbf{Por qué Ian lleva la parte más difícil}]
\small
El tramo final concentra lo conceptualmente más delicado y lo que más probablemente reciba preguntas:
\textbf{(1)} la \textbf{comparación de tiempos de sincronización entre las tres redes} (slide 32), que es la síntesis transversal de todo el trabajo;
\textbf{(2)} el \textbf{acople crítico $K_c$ dependiente de la conectividad}, que aparece de forma más extrema en el anillo ($k=1$ casi no sincroniza);
\textbf{(3)} el \textbf{experimento de condiciones iniciales} (slide 33) y su cierre en las \textbf{conclusiones} (slide 35), que exige manejar el argumento estadístico. Ian debe dominar el apéndice de preguntas frecuentes al final de este guion.
\end{tcolorbox}
...
\textit{\small Esta es la parte más difícil de la presentación: la síntesis entre redes y las conclusiones. Leer el apéndice de preguntas frecuentes antes de presentar.}
