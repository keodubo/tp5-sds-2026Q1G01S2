...
param(
    [int]$Threads = 5,
    [string]$OutputDir = "outputs/fhn-sweep-T500-dt005-init05-observables",
    [long]$BaseSeed = 20260607,
    [switch]$Overwrite,
    [switch]$DryRun
)
...
try {
    Write-Host "TP5 FHN sweep"
    Write-Host "  output_dir: $OutputDir"
    Write-Host "  threads:    $Threads"
    Write-Host "  base_seed:  $BaseSeed"
    Write-Host "  initial:    v_i, w_i uniform in [-0.5, 0.5]"
    Write-Host "  seeds:      distinct runSeed per parameter combination and realization"
    ...
    $javaArgs = @(
        "-cp", $buildDir,
        "ar.edu.itba.sds.tp5.Main", "sweep",
        "--topology", "all",
        "--N", "501",
        "--T", "500",
        "--dt", "0.005",
    ...
    Write-Host "Starting resumable sweep. Existing complete runs are skipped unless -Overwrite is used."
    & java @javaArgs
    if ($LASTEXITCODE -ne 0) {
